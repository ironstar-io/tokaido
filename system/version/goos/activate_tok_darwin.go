...
func ActivateSavedVersion(version string) bool {
	...
	fs.Remove(constants.GlobalBinaryPathDarwin)

	// Copy the specified version to /usr/local/bin/tok
	fs.Copy(p, constants.GlobalBinaryPathDarwin)

	// Make sure the version is executable
	err := os.Chmod(constants.GlobalBinaryPathDarwin, 0777)
	if err != nil {
		fmt.Println("Unexpected error granting execute permissions to [" + constants.GlobalBinaryPathDarwin + "]: " + err.Error())
		os.Exit(1)
	}

...
func CreateSymlink(path string) error {
	...
	if err != nil {
		...
		utils.DebugString("The Go native method failed to create a symbolic link. Usually this is due to symlink creation requiring elevation. Falling back to attempt `mklink /d`")
		utils.DebugErrOutput(err)
